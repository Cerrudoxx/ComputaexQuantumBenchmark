...
def main():
    """The main function for running the QFT algorithm with Qiskit."""
    ...
    if '-' in args.num_iterations:
        ...
        if start >= end or start <= 0 or (start & (start - 1)) != 0:
            print("Error: Invalid range of iterations. Start must be a power of 2 and less than end.")
            sys.exit(1)
        iterations_list = []
        current = start
        while current <= end:
        ...
        if num_iterations <= 0 or (num_iterations & (num_iterations - 1)) != 0:
            print("Error: Number of iterations must be a positive power of 2.")
            sys.exit(1)
    ...
    times_file_name = f'QFT_data_qiskit_{args.n}'
    ...
    for n in qubits_list:
        for num_iterations in iterations_list:
            console.print(f"Running QFT algorithm with {n} qubits, {num_iterations} iterations, and {args.cores} cores...", style="bright_magenta")
