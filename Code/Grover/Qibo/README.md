# Grover's Algorithm Simulation with Qibo
...
```bash
python3 qbench.py run grover qibo [OPTIONS]
```
...
```bash
python3 qbench.py run grover qibo --qubits 4 --iterations 512
```

Run with a qubit range from 4 to 7, iterations from 512 to 1024, and 2 cores:
```bash
python3 qbench.py run grover qibo --qubits 4-7 --iterations 512-1024 --cores 2
```
...
- **Purpose**: Constructs and executes the quantum circuit for Grover's algorithm in Qibo.
- **Key Methods**: 
  - `run()`: Executes the algorithm using the unified `Qibo` backend. Stops execution if the estimated time exceeds one day.
...
- **Results CSV** (`Grover_data_qibo_<n>.csv`): Contains comprehensive execution data.
- **Plots**:
