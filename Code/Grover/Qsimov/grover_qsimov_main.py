...
def main():
    """The main function for running Grover's algorithm with Qsimov."""
    ...
    parser.add_argument("num_iterations", type=str, help="Number of iterations or range (e.g., '512' or '512-1024'). It is recommended that the range is in powers of 2")
    ...
    if '-' in args.num_iterations:
        ...
        if start >= end:
            ...
            sys.exit(1)
        iterations_list = [2**i for i in range(start.bit_length() - 1, end.bit_length()) if 2**i >= start and 2**i <= end]
    else:
